...
public final class GrpcClientApp {
	...
	private static void demonstrateUnaryRpc(GrpcClient client) {
		...
		try {
			MessageDto message = new MessageDto("user123", "Hello from gRPC client!", Instant.now());
			MessageResponseDto response = client.sendMessage(message);
			logger.info("Message sent successfully: messageId={}, status={}", response.messageId(), response.status());
		} catch (Exception e) {
	...
	private static void demonstrateServerStreaming(GrpcClient client) {
		...
		try {
			...
			notifications.forEach(n -> logger.info("  - [{}] {}: {}", n.priority(), n.topic(), n.title()));
	...
	private static void demonstrateClientStreaming(GrpcClient client) {
		...
		try {
			...
			logger.info("Aggregate received: totalReadings={}, avg={}, min={}, max={}", aggregate.totalReadings(),
					aggregate.averageValue(), aggregate.minValue(), aggregate.maxValue());
		} catch (InterruptedException e) {
			Thread.currentThread().interrupt();
